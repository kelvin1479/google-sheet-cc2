...
public class ExpParser {
    ...
    private double parseAtomElement() throws Exception {
        ...
        switch (this.tokenType) {
            ...
            case FUNCTION_TOKEN:
            	...
            	result = fc.getResult();
            	if(!fc.valid) valid = false;
            	if(!fc.valid_strong) valid_strong = false;
            	NotZeroCell += fc.NotZeroCell;
            	this.getToken();
            	break;
            default:
                this.handleError(SYNTAX_ERROR);
    ...
    private double findVar(String vname) throws Exception {
    	...
    	if(row < 0 || column < 0 || row >= sheetReader.getRowCount() || column >= sheetReader.getColCount()) {
    		valid = false;
    		valid_strong = false;
    		return 0;
    	}
    	if(sheetReader.getCells()[row][column].getCellType() == Cell.TYPE_FORMULA)
    		valid = false;
    	if(sheetReader.getCells()[row][column].getCellType() == 1) {
    		//System.err.println("AAA:"+Double.parseDouble(sheetReader.getCells()[row][column].getValue()));
    		double tmp = Double.parseDouble(sheetReader.getCells()[row][column].getValue());
    		if(tmp != 0) NotZeroCell++;
    		return tmp;
    	}
    	else {
    		valid = false;
    		if(sheetReader.getCells()[row][column].getValue() != null && sheetReader.getCells()[row][column].getValue().length() > 2)
    			valid_strong = false;
    		return 0;
    	}
    }
